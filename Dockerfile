...
RUN mkdir -p spec && make BUILD_FLAGS="--verbose --stats --progress --time --threads 8 --release --static --mcpu generic"
...
COPY --from=build_and_test /app/out/meshcore-tcp-mux /usr/local/bin/meshcore-tcp-mux
